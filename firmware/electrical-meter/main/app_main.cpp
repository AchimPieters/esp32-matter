/*
 * Minimal Matter Electrical Meter — forty-sixth device type, and this
 * repo's first over the Electrical Meter device type itself: a standalone
 * whole-circuit/whole-home energy meter, reusing firmware/outlet/'s own
 * 6-chip power-monitoring subsystem verbatim as the device's own primary
 * function rather than a bonus feature bolted onto a plug-in outlet.
 *
 ...
 *
 * --- Device type + scope: a complete top-level helper, one endpoint -----
 * Confirmed directly against the CSA's own data_model/1.6/device_types/
 * ElectricalMeter.xml (device type 0x0514, revision 1): Electrical Power
 * Measurement and Electrical Energy Measurement are both mandatoryConform;
 * Commodity Metering is `provisionalConform` in its own fallback branch —
 * not yet implementable against a real certified product, the same
 * "provisionalConform, don't implement yet" precedent firmware/
 * microwave-oven/'s own PowerInWatts feature already establishes.
 * `endpoint::electrical_meter::create()` confirmed complete/ready-to-use
 * by reading `esp_matter_endpoint.cpp`'s own `electrical_meter::add()`
 * directly: it wires up Descriptor (via `common::create<T>()`) +
 * ElectricalPowerMeasurement + ElectricalEnergyMeasurement, all from ONE
 * `config_t` — unlike firmware/outlet/'s own two-endpoint composition
 * (an `on_off_plug_in_unit` endpoint plus a separate `electrical_sensor`
 * endpoint for its optional power-monitoring feature), this device type
 * needs only the one endpoint, since being a meter is its entire purpose
 * rather than an add-on.
 *
 * --- Power monitoring: firmware/outlet/'s own 6-chip subsystem, reused
 * verbatim, always enabled (not optional) --------------------------------
 * `ELECTRICAL_METER_CHIP` is the exact same 6-way choice firmware/
 * outlet/'s own `OUTLET_POWER_MONITOR` already offers (BL0942, BL0937,
 * HLW8012, CSE7759, CSE7766, ADE7953) — every driver, protocol detail,
 * and sourcing citation reused unchanged from that file (see its own
 * header comment for the complete per-chip protocol/formula/datasheet
 * detail; not repeated here to avoid duplicating ~150 lines of citation
 * text verbatim). The one real difference: outlet's own equivalent
 * `#define` defaults to `OUTLET_POWER_MONITOR_NONE` (power monitoring is
 * an optional bonus on a plug-in outlet) — this device type has no
 * "none" option at all, since a meter with no monitoring chip selected
 * would be a non-functional device; `ELECTRICAL_METER_CHIP` defaults to
 * `_BL0942` (the most accurate of the six — a real UART request/response
 * protocol with on-chip calibration, rather than a board-calibrated
 * pulse-frequency GPIO chip).
 *
 * Like firmware/outlet/'s own header comment documents in full,
 * ElectricalPowerMeasurement uses a *Delegate* interface reached via a
 * manually-constructed `Instance` (not the newer `config->delegate` +
 * automatic-init-callback mechanism `ElectricalPowerMeasurementDelegateInitCB`
 * provides) — confirmed this is NOT a missed shortcut this time (unlike
 * firmware/doorbell/'s and firmware/chime/'s own Chime-cluster correction
 * earlier this session): esp-matter's own official reference
 * (`examples/all_device_types_app/main/electrical_measurement/
 * electrical_measurement.cpp`, read directly) uses the exact same manual
 * `std::unique_ptr<ElectricalPowerMeasurement::Instance>` construction
 * pattern outlet's own file already follows, confirming this is the
 * intended, reference-grounded approach for this specific cluster, not a
 * research gap — the automatic `config->delegate` path exists too, but
 * needs its own optional-attribute ember bookkeeping
 * (`attribute::create_rms_voltage()`/`create_rms_current()`) the official
 * reference itself doesn't use, so it wasn't adopted here either.
 * ElectricalEnergyMeasurement, by contrast, still uses esp-matter's own
 * complete ready-made free-function API (`SetMeasurementAccuracy()`,
 * `NotifyCumulativeEnergyMeasured()`) — no custom Delegate needed, same
 * as firmware/outlet/'s own identical integration.
 *
 ...
 * available when written — same standard firmware/outlet/'s own header
 * comment already documents for its identical drivers).
 */
...
static const char *TAG = "matter_electrical_meter";
...
#define ELECTRICAL_METER_CHIP_BL0942 1
#define ELECTRICAL_METER_CHIP_BL0937 2
#define ELECTRICAL_METER_CHIP_HLW8012 3
#define ELECTRICAL_METER_CHIP_CSE7759 4
#define ELECTRICAL_METER_CHIP_CSE7766 5
#define ELECTRICAL_METER_CHIP_ADE7953 6
#define ELECTRICAL_METER_CHIP ELECTRICAL_METER_CHIP_BL0942

#if ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_BL0942
...
#elif ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_BL0937 || \
      ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_HLW8012 || \
      ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_CSE7759
...
#if ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_BL0937
...
#elif ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_CSE7766
...
#elif ELECTRICAL_METER_CHIP == ELECTRICAL_METER_CHIP_ADE7953
...
static uint16_t meter_endpoint_id = 0;
...
static void report_power(double watts, double volts, double amps, int64_t delta_energy_mwh)
{
    ...
    NotifyCumulativeEnergyMeasured(meter_endpoint_id, imported_nullable, exported_nullable);
...
extern "C" void app_main(void)
{
    ...
    /* 3. Build the Matter data model: one node, one Electrical Meter
     * endpoint (Descriptor + ElectricalPowerMeasurement +
     * ElectricalEnergyMeasurement, all via the complete top-level helper
     * — see the header comment above). */
    node::config_t node_config;
    strncpy(node_config.root_node.basic_information.node_label, "Electrical Meter",
    ...
    meter_config.electrical_power_measurement.feature_flags =
        chip::to_underlying(chip::app::Clusters::ElectricalPowerMeasurement::Feature::kAlternatingCurrent);
    endpoint_t *endpoint = endpoint::electrical_meter::create(node, &meter_config, ENDPOINT_FLAG_NONE, NULL);
    if (!endpoint) {
        ESP_LOGE(TAG, "Failed to create electrical meter endpoint");
        return;
    }
    meter_endpoint_id = endpoint::get_id(endpoint);
    ESP_LOGI(TAG, "Electrical meter endpoint id: %u", meter_endpoint_id);

    /* 3a. Identify — optionalConform (confirmed against the CSA's own XML
     * — actually not even listed there at all, so this is purely an
     * added convenience, same as every other device type in this repo
     * ships one). */
    ...
    SetMeasurementAccuracy(meter_endpoint_id, energy_accuracy);
    ...
    power_instance = new chip::app::Clusters::ElectricalPowerMeasurement::Instance(
        meter_endpoint_id, power_delegate, features, optional_attrs);
    ...
    ESP_LOGI(TAG, "Matter electrical meter started. Scan the QR code to commission.");
}
