...
/* Warm/cool white physical color temperature — see the header comment for
 * the W1=warm/W2=cool (or, for SM2335EGH, OUT4=warm/OUT5=cool) convention
 * this assumes. 6500K/2700K are the same two most common "daylight"/"warm
 * white" LED bin ratings firmware/color-light/'s own RGBWW mode uses, for
 * the same reasoning (see that file's header comment) — adjust to your
...
/* Optional RGB status LED — off by default (GPIO_NUM_NC). This is a
 * SEPARATE, plain 3-channel LEDC-PWM indicator LED, not the strip/fixture
 * itself — unrelated to identify_via_strip above, which only governs how
 * plain Identify blinking is shown. Uses LEDC_TIMER_1 + channels 5-7; this
 * file drives its strip/fixture via RMT/SPI/bit-banged GPIO, never LEDC, so
 * there's no collision to check against. See firmware/light/main/app_main.cpp's
 * header comment for the full state list and its exact sourcing
 * (connectedhomeip's own lifecycle events + the Identify cluster's own
 * EffectIdentifierEnum), not repeated here. */
 ...
 * on the RGB/CCT interlock (identical concept and code shape to
 * firmware/color-light/'s COLOR_LIGHT_MODE_RGBWW). */
...
/* Computes the current on/off + level + hue/saturation (or, for the
 * RGBCCT chips, color temperature) result into up to 5 [0,1] fractions —
 * shared by both the strip-flash Identify path and the real set_output()
 * below, so Identify always flashes using the same color math this device
...
static void app_event_cb(const ChipDeviceEvent *event, intptr_t arg)
{
    ...
    case chip::DeviceLayer::DeviceEventType::kCHIPoBLEAdvertisingChange:
        if (event->CHIPoBLEAdvertisingChange.Result == chip::DeviceLayer::kActivity_Started) {
            status_led_set(0, 255, 0, STATUS_LED_PATTERN_BREATHE, 2000, 0);
        }
        break;
    case chip::DeviceLayer::DeviceEventType::kSecureSessionEstablished:
        status_led_set(255, 255, 0, STATUS_LED_PATTERN_BREATHE, 1000, 0);
        break;
    ...
    case chip::DeviceLayer::DeviceEventType::kFailSafeTimerExpired:
        status_led_set(255, 0, 0, STATUS_LED_PATTERN_SOLID, 1000, 0);
        break;
    default:
        break;
    }
...
extern "C" void app_main(void)
{
     ...
     * GPIO_NUM_NC default). This is a plain LEDC-PWM indicator, unrelated
     * to the strip/fixture output above (RMT/SPI/bit-banged GPIO, never
     * LEDC), so there's nothing to collide with. */
    ...
    /* CurrentLevel/CurrentHue/CurrentSaturation (and ColorTemperatureMireds
     * for the RGBCCT chips) all change rapidly while a controller is
