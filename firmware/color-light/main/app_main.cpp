...
/* Optional RGB status LED — off by default (GPIO_NUM_NC). Reuses
 * LEDC_TIMER_1 and channels 5-7, deliberately separate from
 * COLOR_LIGHT_LEDC_TIMER/its own channels 0-4 (LEDC_TIMER_0) below, so it
 * never collides with the light's own color output regardless of which
 * COLOR_LIGHT_COLOR_MODE is active. See firmware/light/main/app_main.cpp's
